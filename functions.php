...
?>
    ...
    <form id="custom-search-filter" action="<?php //echo esc_url(home_url('/camp-results/')); ?>" method="POST" enctype="multipart/form-data">
    	...
    	<div class="block">
		    ...
		    <select name="camp_type" id="camp_type" class="select-js">
		...
		<div class="block">
		    ...
		    <select name="division" id="division" class="select-js">
    	...
    	<div class="block">
		    ...
		    <select name="gender" id="gender" class="select-js">
    	...
    	<div class="block">
		    ...
		    <select name="region" id="region" class="select-js">
		...
		<div class="block">    
    		...
    		<select name="month" id="month" class="select-js">
		...
		<div class="block">    
    		...
    		<select name="school" id="school" class="select-js">
		...
		<div class="block">    
		    ...
		    <select name="state" id="state" class="select-js">
...
function custom_ajax_function_test() 
{
    if (isset($_POST['formData'])) {
        ...
        // Decode the JSON string to an associative array (if necessary)
        // $formDataArray = json_decode($formData, true);

        $campType = sanitize_text_field($formData[0]['value']);
        $division = sanitize_text_field($formData[1]['value']);
        $gender = sanitize_text_field($formData[2]['value']);
        $region = sanitize_text_field($formData[3]['value']);
        $month = sanitize_text_field($formData[4]['value']);
        $school = sanitize_text_field($formData[5]['value']);
        $state = sanitize_text_field($formData[6]['value']);
        ...
        if ($ajaxquery->have_posts()) {
            ...
            while ($ajaxquery->have_posts()) {
                ...
                if (strtotime($camp_date) >= strtotime(date('F j, Y'))) {
                    // Store the post ID and camp_date in the results array
                    $results[] = array('post_id' => $post_id, 'camp_date' => $camp_date);
                }
            }

            // Function to sort the results array by camp_date
            ...
            $output = '';
            $output .= '<div class="camp_card_main">';
            
            foreach ($results as $result) {
                $post_id = $result['post_id'];
                $camp_date = $result['camp_date'];
                $city_state = get_field('city__state', $post_id);
                $camp_loc = get_field('camp_loc', $post_id);
                $camp_player = get_field('camp_player', $post_id);
                $camp_tour = get_field('camp_name', $post_id);
                $golf_course = get_field('course_name', $post_id);
                $college_coach = get_field('number_of_coaches', $post_id);
                $school_logos = get_field('school_logos', $post_id);

                $school_logo = !empty($school_logos['url']) ? $school_logos['url'] : home_url('/wp-content/uploads/2024/03/Tarheel_Junior_Golf_Tour.png');
                $campqualify = get_field('camp_qualifires', $post_id);
                $stypad = $campqualify === 'Yes' ? 'padding-bottom: 65px;' : '';
                $campqualifyimg = $campqualify === 'Yes' ? home_url('/wp-content/uploads/2024/05/TRUE.png') : '';
                ...
                $output .= '<div class="camp_title"><a href="' . get_permalink($post_id) . '">' . get_the_title($post_id) . '</a> <span>' . $city_state . '</span> </div>';
                ...
                if (!empty($camp_tour)) {
                    $output .= '<div class="camp_play"> <span>Campus Tour: </span>' . $camp_tour . '</div>';
                }
                
                if (!empty($golf_course)) {
                    $output .= '<div class="camp_play"> <span>Golf Course: </span>' . $golf_course . '</div>';
                }
                
                if (!empty($college_coach)) {
                    $output .= '<div class="camp_play"> <span>College Coaches: </span>' . $college_coach . '</div>';
                }
                ...
                $output .= '</div>';
                $output .= '<div class="camp_btn" style="' . $stypad . '"><a class="et_pb_button dipi-button-grid dipi-button-wrap" href="' . get_permalink($post_id) . '" data-icon="5">CAMP INFO</a></div>';
            
                if (!empty($campqualifyimg)) {
                    $output .= '<div class="trueimg"><img src="' . $campqualifyimg . '" alt=""></div>';
                }
        ...
        // Echo the output and terminate the script
        echo $output;
    }
...
// function custom_ajax_function_test_old() 
// {
...
function grab_camp_listings()
{
	...
	if (!empty($_SERVER['QUERY_STRING'])) 
	{
		...
		$campType = isset($queryParams['camp_type']) ? $queryParams['camp_type'] : null;
	 	$division = isset($queryParams['division']) ? $queryParams['division'] : null;
        $gender = isset($queryParams['gender']) ? $queryParams['gender'] : null;
        $region = isset($queryParams['region']) ? $queryParams['region'] : null;
        $month = isset($queryParams['month']) ? $queryParams['month'] : null;
        $school = isset($queryParams['school']) ? $queryParams['school'] : null;
